...
import { Section, Heading, FooterMenu, CountrySelector } from '..';
...
export function Footer({ menu }: { menu?: EnhancedMenu }) {
    ...
    let itemsCount: any = [];
    if (menu) {
        if (menu.items && menu.items.length && menu.items.length + 1 > 4) {
            itemsCount = 4;
        } else {
            itemsCount = menu.items.length + 1;
        }
    }
    ...
    return (
        ...
        >
            ...
            <div className={`self-end pt-8 opacity-50 md:col-span-2 lg:col-span-${itemsCount}`}>
                &copy; {new Date().getFullYear()} / Shopify, Inc. Hydrogen is an MIT Licensed Open Source project. This
