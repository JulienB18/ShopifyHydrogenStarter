...
import { Button, Text, ProductCard, Heading, Skeleton } from '..';

function TopProducts({ onClose }: { onClose?: () => void }) {
    const response = fetchSync('/api/bestSellers');

    if (!response.ok) {
        console.error(`Unable to load top products ${response.url} returned a ${response.status}`);
        return null;
    }
...
export function CartEmpty({ onClose, layout = 'drawer' }: { onClose?: () => void; layout?: 'page' | 'drawer' }) {
    ...
    return (
        <div ref={scrollRef} className={container[layout]}>
            ...
            <section className="grid gap-8 pt-4">
                ...
                <div className={`grid grid-cols-2 gap-x-6 gap-y-8 ${topProductsContainer[layout]}`}>
