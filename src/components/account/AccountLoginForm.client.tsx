...
export function AccountLoginForm({ shopName }: { shopName: string }) {
    ...
    async function checkPassword(event: React.FormEvent<HTMLFormElement & FormElements>) {
        const { validity } = event.currentTarget.password;
        if (validity.valid) {
            ...
            setPasswordError(
                validity.valueMissing ? 'Please enter a password' : 'Passwords must be at least 6 characters',
            );
        }
    }
                    ...
                    {hasSubmitError && (
                        <div className="flex items-center justify-center mb-6 bg-zinc-500">
                            <p className="m-4 text-s text-contrast">
                                Sorry we did not recognize either your email or password. Please try to sign in again or
                                create a new account.
                            </p>
                        </div>
                    )}
                    {showEmailField && (
                        <EmailField shopName={shopName} email={email} setEmail={setEmail} emailError={emailError} />
                    )}
