...
import { emailValidation, passwordValidation, useRenderServerComponents } from '../../lib/utils';
...
function Password({ name, passwordError, label }: { name: string; passwordError: string | null; label: string }) {
...
export function AccountDetailsEdit({
    ...
    async function onSubmit(event: React.FormEvent<HTMLFormElement & FormElements>) {
        ...
        let password2ErrorNew;

        // Only validate the password fields if the current password has a value
        if (event.currentTarget.currentPassword.value) {
            newCurrentPasswordError = passwordValidation(event.currentTarget.currentPassword);
            if (newCurrentPasswordError) {
                setCurrentPasswordError(newCurrentPasswordError);
            }

            passwordErrorNew = passwordValidation(event.currentTarget.newPassword);
            if (passwordErrorNew) {
                setNewPasswordError(passwordErrorNew);
            }
            ...
            password2ErrorNew =
                event.currentTarget.newPassword.value !== event.currentTarget.newPassword2.value
                    ...
                    : null;
            if (password2ErrorNew) {
                setNewPassword2Error(password2ErrorNew);
            }
        }
