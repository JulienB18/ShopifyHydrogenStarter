...
import { IconCaret, IconCheck } from './index';
...
export function Countries({
    ...
    if (response.ok) {
        ...
        console.error(`Unable to load available countries ${response.url} returned a ${response.status}`);
    }
        ...
        countries.map((country) => {
            ...
            return (
                <Listbox.Option key={country.isoCode} value={country}>
                    {({ active }) => (
                        <div className={`text-contrast dark:text-primary ${getClassName(active)}`}>
                            {country.name}
                            {isSelected ? (
                                <span className="ml-2">
                                    <IconCheck />
        ...
        ({ isoCode: newIsoCode }) => {
            ...
            if (newIsoCode !== 'US') {
                if (currentCountry.isoCode === 'US') {
                    ...
                    redirectPath = `/${newIsoCode.toLowerCase()}${currentPath.substring(currentPath.indexOf('/', 1))}`;
                }
            } else {
