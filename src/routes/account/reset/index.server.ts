...
import { CUSTOMER_RESET_MUTATION } from './query';
...
export async function api(request: HydrogenRequest, { session, queryShop }: HydrogenApiRouteOptions) {
    ...
    if (!jsonBody.id || !jsonBody.password || !jsonBody.resetToken) {
        return new Response(JSON.stringify({ error: 'Incorrect password or reset token.' }), {
            status: 400,
        });
    }
    ...
    if (data?.customerReset?.customerAccessToken?.accessToken) {
        await session.set('customerAccessToken', data.customerReset.customerAccessToken.accessToken);
