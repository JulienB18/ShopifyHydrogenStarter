...
export default function OrderDetails({ response }: HydrogenRouteProps) {
    ...
    const [order] = flattenConnection<Order>(data?.customer?.orders ?? {}) || [null];
    ...
    const firstDisccountTypeName = firstDiscount?.__typename;
    const discountValue = firstDisccountTypeName === 'MoneyV2' && firstDiscount;
    const discountPercentage = firstDisccountTypeName === 'PricingPercentageValue' && firstDiscount?.percentage;
