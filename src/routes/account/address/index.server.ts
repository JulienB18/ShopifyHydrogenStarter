...
import { CREATE_ADDRESS_MUTATION } from './query';
...
export async function api(request: HydrogenRequest, { session, queryShop }: HydrogenApiRouteOptions) {
    ...
    if (isDefaultAddress) {
        const { data: defaultDataResponse, errors: newErrors } = await setDefaultAddress(
            queryShop,
            data.customerAddressCreate.customerAddress.id,
            customerAccessToken,
        );

        const newError = getApiErrorMessage('customerDefaultAddressUpdate', defaultDataResponse, newErrors);
