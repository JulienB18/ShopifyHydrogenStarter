import { CacheNone, type HydrogenApiRouteOptions, type HydrogenRequest } from '@shopify/hydrogen';

import { getApiErrorMessage } from '../../../lib/utils';
import type { Address } from './index.server';
import { DELETE_ADDRESS_MUTATION, UPDATE_ADDRESS_MUTATION, UPDATE_DEFAULT_ADDRESS_MUTATION } from './query';
...
async function updateAddress(
    ...
    const { firstName, lastName, company, address1, address2, country, province, city, zip, phone, isDefaultAddress } =
        await request.json();
    ...
    if (isDefaultAddress) {
        const { data: newData, errors: newErrors } = await setDefaultAddress(
            queryShop,
            decodeURIComponent(params.addressId ?? ''),
            customerAccessToken,
        );

        const newError = getApiErrorMessage('customerDefaultAddressUpdate', newData, newErrors);

        if (newError) return new Response(JSON.stringify({ newError }), { status: 400 });
    }

    return new Response(null);
}

export async function api(request: HydrogenRequest, { params, session, queryShop }: HydrogenApiRouteOptions) {
    if (!session) {
    ...
    if (request.method === 'PATCH') return updateAddress(customerAccessToken, request, params, queryShop);
