...
import { Menu, MenuItem, MoneyV2, UserError } from '@shopify/hydrogen/storefront-api-types';
...
export function getApiErrorMessage(field: string, data: Record<string, any>, errors: UserError[]) {
    if (errors && errors.length > 0) return errors[0]?.message ?? errors[0];
